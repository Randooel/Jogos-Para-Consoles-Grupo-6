...
public class Consumidor {
    ...
    public void consumirRecurso(Armazen armazem) {
        new Thread(() -> {
            while (true) {
                try {
                    // Tempo necessário para consumir um recurso com base na velocidade de consumo
                    Thread.sleep((long) (velocidadeConsumo * 1000));

                    if (!armazem.getItens().isEmpty()) {
                        // Remove o recurso mais antigo ou pode implementar uma lógica de prioridade
                        String recurso = armazem.getItens().remove(0);
                        System.out.println(nome + " consumiu o recurso: " + recurso);

                        //Lógica para usar o recurso consumido, tipo construir um item
